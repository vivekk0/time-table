...
def challenge():
    ...
    if user == "Student":
        ...
        cursor = list(cursor)
        if len(cursor) == 0:
            messagebox.showwarning('Wrong id', 'No such user found!')
        elif passw_entry.get() != cursor[0][0]:
            messagebox.showerror('Wrong pass', 'Incorret Password!')
        else:
            nw = tk.Tk()
            tk.Label(
                nw,
        ...
        if id_entry.get() == 'admin' and passw_entry.get() == 'admin':
            ...
            messagebox.showerror('Wrong Input', 'Incorret Username/Password!')
